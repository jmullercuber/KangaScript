...
# partyList, slightly edited from PSet 3.2
def partyList(invited, attending=[]):
    ...
    if len(invited) == 0:
        ...
        #print "Going deeper"
        return [] + partyList(invited[1:], attending + invited[:1]) + partyList(invited[1:], attending)

def minElem(listy):
    mins_index = 0
    min_len = len(listy[0])
    for i in range(1, len(listy)):
        if len(listy[i]) < min_len:
            mins_index = i
            min_len = len(listy[i])
    return listy[mins_index]

def autodebug(test, interesting): 
        # find the smallest subset of test that is still interesting!
        if not interesting(test):
                return None
        subset = filter( interesting, partyList(test) )
        return minElem(subset)
        # your code here ...
